...
public class SpreadableSnowyAljanDirtBlock extends SnowyDirtBlock {
    ...
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        if (!isSnowyConditions(state, worldIn, pos)) {
            ...
            worldIn.setBlockState(pos, BMBlocks.ALJAMIC_DIRT.get().getDefaultState());
        } else {
            if (worldIn.getLight(pos.up()) >= 9) {
                ...
                for(int i = 0; i < 4; ++i) {
                    ...
                    if (worldIn.getBlockState(blockpos).isIn(BMBlocks.ALJAMIC_DIRT.get()) && isSnowyAndNotUnderwater(blockstate, worldIn, blockpos)) {
