...
public class DevilSpareyItem extends SwordItem implements ToolBehaviors {
    ...
    public boolean onLeftClickEntity(ItemStack stack, PlayerEntity player, Entity target) {
        if (target.getType().is(inSpareyProhibitedTag(stack))) {
            ...
            player.addEffect(getSpareyEffect(new EffectInstance(Effects.DAMAGE_BOOST, 50, 2), stack, player.level, "strength_effect"));
            ...
            player.addEffect(getSpareyEffect(new EffectInstance(Effects.WEAKNESS, 200, 1), stack, player.level, "weakness_effect"));
