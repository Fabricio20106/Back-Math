    ...
    public void render(int ticks, float partialTicks, MatrixStack matrixStack, ClientWorld world, Minecraft mc) {
        if (mc.world.getDimensionKey() == BMDimensions.THE_ALJAN) {
            ...
            if (afloat != null) {
                ...
                float f3 = MathHelper.sin(world.getCelestialAngleRadians(partialTicks)) < 0 ? 180 : 0;
                ...
                bufferBuilder.pos(matrix4F, 0, 100, 0).color(f4, f5, f6, afloat[3]).endVertex();

                for(int j = 0; j <= 16; ++j) {
                    float f7 = (float)j * ((float) Math.PI * 2F) / 16;
                    float f8 = MathHelper.sin(f7);
                    float f9 = MathHelper.cos(f7);
                    bufferBuilder.pos(matrix4F, f8 * 120, f9 * 120, -f9 * 40 * afloat[3]).color(afloat[0], afloat[1], afloat[2], 0).endVertex();
                }
            ...
            matrixStack.push();
            float f11 = 1 - world.getRainStrength(partialTicks);
            RenderSystem.color4f(1, 1, 1, f11);
            matrixStack.rotate(Vector3f.YP.rotationDegrees(-90));
            matrixStack.rotate(Vector3f.XP.rotationDegrees(world.func_242415_f(partialTicks) * 360));
            Matrix4f matrix4F = matrixStack.getLast().getMatrix();

            mc.textureManager.bindTexture(ALJAN_SUN_TEXTURES);
            bufferBuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
            bufferBuilder.pos(matrix4F, -30, 100, -30).tex(0, 0).endVertex();
            bufferBuilder.pos(matrix4F, 30, 100, -30).tex(1, 0).endVertex();
            bufferBuilder.pos(matrix4F, 30, 100, 30).tex(1, 1).endVertex();
            bufferBuilder.pos(matrix4F, -30, 100, 30).tex(0, 1).endVertex();
            bufferBuilder.finishDrawing();
            WorldVertexBufferUploader.draw(bufferBuilder);
            RenderSystem.disableTexture();
    ...
    private void renderSky(BufferBuilder bufferBuilder, float posY, boolean reverseX) {
        ...
        for(int k = -384; k <= 384; k += 64) {
            for(int l = -384; l <= 384; l += 64) {
                ...
                bufferBuilder.pos(f, posY, l).endVertex();
                bufferBuilder.pos(f1, posY, l).endVertex();
                bufferBuilder.pos(f1, posY, (l + 64)).endVertex();
                bufferBuilder.pos(f, posY, (l + 64)).endVertex();
            }
        }
